...
for numGen = 100:100:2000
    for runs = 1:15
        ...
        resultStruct = tsp_ga2(userConfig);
...
% plots ...
newResults = zeros(20,3);
for calc = 1:20
    SEM = std(results(calc,:))/sqrt(length(results(calc,:)));	% Standard Error
    ts = tinv([0.025  0.975],length(results(calc,:))-1);        % T-Score
    CI = mean(results(calc,:)) + ts*SEM;                        % Confidence Intervals
    newResults(calc,1) = CI(1);
    newResults(calc,2) = CI(2);
    newResults(calc,3) = mean(results(calc,:));
end
